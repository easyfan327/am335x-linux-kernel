...
int add_remove_lp_flag = 0;
int usbprint_vid = 0;

static int send_to_kt_user(int msg_type, int data);
...
static void usblp_cleanup (struct usblp *usblp)
{
	...
	//=============masda20120912===================
	if (1 == add_remove_lp_flag)
	{
		add_remove_lp_flag = 0;
		send_to_kt_user(REMOVE_MSG, 0);
		usbprint_vid = 0;
	}
	//====================end====================
		       ...
		       const struct usb_device_id *id)
{
	...
	//=============masda20120912===================
	if (0 == add_remove_lp_flag)
	{
		add_remove_lp_flag = 1;
		send_to_kt_user(LP_ADD_MSG, le16_to_cpu(usblp->dev->descriptor.idProduct));
		usbprint_vid = le16_to_cpu(usblp->dev->descriptor.idProduct);
	}
	//====================end====================
...
void usblp_nlskrecv_handler(struct sk_buff *__skb)
{
	struct sk_buff *skb;
	struct nlmsghdr *nlh;
	//获取对skb的引用指针
	...
	if( nlh->nlmsg_type == INIT_LP_MSG )
	{		
		kt_user_pid = nlh->nlmsg_pid;	
		//printk("Rec kt_user_pid = %d!!!\n", kt_user_pid);
		if ((1 == add_remove_lp_flag) && (usbprint_vid != 0))
		{
			send_to_kt_user(LP_ADD_MSG, usbprint_vid);
		}
	}
	else
	{
		kt_user_pid = 0;
		printk("No Rec kt_user_pid\n");
		printk("Rec nlmsg_type = %d\n", nlh->nlmsg_type);
	}
	//printk("msg:%s\n",(char *)NLMSG_DATA(nlh));
free:
	kfree_skb(skb);
...
static int send_to_kt_user(int msg_type, int data)
{
	...
	/*初始化一个netlink消息首部*/
	//nlh = NLMSG_PUT(skb, 0, 0, msg_type, size-sizeof(*nlh));
	...
	if (0 != data)
	{
		#if 0
		/*跳过消息首部，指向数据区*/
		databuf = NLMSG_DATA(nlh);
		if (NULL == databuf)
		{
        	return -1;  
		}
		/*填充待发送的数据*/
		sprintf(databuf,"0x%4.4X", data);
		#endif
		sprintf(NLMSG_DATA(nlh),"0x%4.4X", data);
	}
	...
	/*设置控制字段*/
 	...
 	//NETLINK_CB(skb).dst_pid = kt_user_pid;
 	NETLINK_CB(skb).pid = 0;   
	...
	//ret = netlink_unicast(nl_sk, skb, kt_user_pid, MSG_DONTWAIT);
	//read_unlock_bh(&user_proc.lock);
    if( netlink_unicast(nl_sk, skb, kt_user_pid, MSG_DONTWAIT) < 0)
	{  
        printk(KERN_ERR "net_link: can not unicast skb /n");  
        return -1;  
    } 
	
		//printk("Usblp Send Data Success!!\n");
...
{ 
	...
	nl_sk = netlink_kernel_create(&init_net, NETLINK_LP_MSG, 0, usblp_nlskrecv_handler, NULL, THIS_MODULE);
	if(!nl_sk)                /*创建失败*/
	{
